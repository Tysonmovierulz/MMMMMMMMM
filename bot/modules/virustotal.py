...
from bot import LOGGER, VIRUSTOTAL_API, VIRUSTOTAL_FREE, app, dispatcher
...
def get_report(file_hash, link=False):
    ...
    try:
        ...
        url = ""
        if link:
            url = baseUrlUrl + "report"
        else:
            url = baseUrlFile + "report"
...
def get_result(file_path):
    ...
    if not file:
        try:
            ...
            url = False
    if file:
        hash = getMD5(path=file_path)
    if not (hash and file):
        hash = file_path
    try:
...
def getResultAsReadable(result):
    ...
    if validateValue(result, "scan_id"):
        someInfo += f"\nScan ID: <code>{result['scan_id']}</code>"
    if validateValue(result, "scan_date"):
        someInfo += f"\nDate: <code>{result['scan_date']}</code>"
    if validateValue(result, "md5"):
        someInfo += f"\nMD5: <code>{result['md5']}</code>"
    if validateValue(result, "sha1"):
        someInfo += f"\nSHA1: <code>{result['sha1']}</code>"
    if validateValue(result, "sha256"):
        someInfo += f"\nSHA256: <code>{result['sha256']}</code>"
    if validateValue(result, "permalink"):
...
def virustotal(update, context):
    if not VIRUSTOTAL_API:
        return LOGGER.error("VirusTotal - VIRUSTOTAL_API not provided.")
    ...
    if message.reply_to_message:
        if message.reply_to_message.document:  # file
            ...
            if message.reply_to_message.document.file_size > maxsize:
                return sendMessage(
                    f"File limit is {humanbytes(maxsize)}", context.bot, update
                )
            try:
                sent = sendMessage(
                    f"Trying to download. Please wait.", context.bot, update
                )
                filename = os.path.join(
                    VtPath, message.reply_to_message.document.file_name
                )
                link = app.download_media(
                    message=message.reply_to_message.document, file_name=filename
                )
            except Exception as e:
                LOGGER.error(e)
        else:
...
virustotal_handler = CommandHandler(
