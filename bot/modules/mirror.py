...
from bot.helper.ext_utils.fs_utils import split as fssplit
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name: str):
        ...
        if LINK_LOGS:
            try:
                ...
                slmsg2 = f"Source Link:\n {source_link}"
                for link_log in LINK_LOGS:
                    bot.sendMessage(
                        link_log, text=slmsg + slmsg2, parse_mode=ParseMode.HTML
                    )
            except IndexError:
                pass
            if reply_to is not None:
        ...
        S_link =  f"https://t.me/c/{link_id}/{msg_id}"
            """
        msg = (
            f'<b>Name: </b><code>{name.replace("<", "")}</code>\n\n<b>Size: </b>{size}'
        )
        ...
        if self.isLeech:
            ...
            if BOT_PM and self.message.chat.type != "private":
                ...
                Thread(
                    target=auto_delete_upload_message, args=(bot, self.message, message)
                ).start()
            ...
            if self.isQbit and QB_SEED and not self.extract:
                if self.isZip:
                    try:
                        ...
                        pass
                msg = sendMarkup(
                    msg + uploader + pmwarn_mirror + warnmsg,
                    self.bot,
                    self.update,
                    InlineKeyboardMarkup(buttons.build_menu(2)),
                )
                Thread(
                    target=auto_delete_upload_message, args=(bot, self.message, msg)
                ).start()
                return
            else:
                try:
...
def _mirror(
    ...
    if FSUB:
        try:
            ...
            if user.status not in ("member", "creator", "administrator"):
                ...
                message = sendMarkup(
                    str(
                        f"<b>Dear {uname}️ You haven't join our Updates Channel yet.</b>\n\nKindly Join @{CHANNEL_USERNAME} To Use Bots. "
                    ),
                    bot,
                    update,
                    reply_markup,
    ...
    if f"{INDEX_URL}" in link:
        link = link.replace(INDEX_URL, ALT_INDEX_URL)
    ...
    if reply_to is not None:
        ...
        if not is_url(link) and not is_magnet(link) or len(link) == 0:
            ...
            if file is None:
                ...
                editMessage(
                    f"<b>{uname}<i> has sent the following TG File</i> - </b>\n<b>Filename:</b> <code>{download_dict[listener.uid].name()}</code>\n<b>Type:</b> <code>{file.mime_type}</code>\n<b>Size:</b> {download_dict[listener.uid].size()}\n\n<b>User ID</b> : <code>{uid}</code> \n\n",
                    tgs,
                )
                sleep(1)
                tgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴛᴇʟᴇɢʀᴀᴍ ғɪʟᴇ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
                # Thread(target=auto_delete_message, args=(bot, update.message, tgs)).start()
                # Thread(target=auto_delete_message, args=(bot, update.message, tgs2)).start()
                return
            else:
    ...
    if (
        ...
        if content_type is None or match(
            ...
            try:
                resp = requests.get(
                    link, timeout=10, headers={"user-agent": "Wget/1.12"}
                )
    ...
    if is_gdrive_link(link):
        ...
        sleep(2)
        editMessage(
            f"<i>{uname} has sent the following GDrive Link</i> - \n\n<code>{link}</code>\n\n<b>User ID</b> : <code>{uid}</code>\n\n",
            gmgs,
        )
        sleep(1)
        ...
        if isZip:
            gmgs2 = sendMessage(
                f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ɢ-ᴅʀɪᴠᴇ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴛʜᴇ sᴛᴀᴛᴜs ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                bot,
                update,
            )
        else:
            gmgs2 = sendMessage(
                f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ɢ-ᴅʀɪᴠᴇ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴛʜᴇ sᴛᴀᴛᴜs ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                bot,
                update,
            )

        # Thread(target=auto_delete_message, args=(bot, update.message, gmgs)).start()
        # Thread(target=auto_delete_message, args=(bot, update.message, gmgs2)).start()
        ...
        if link_type == "folder" and BLOCK_MEGA_FOLDER:
            ...
            if MEGAREST:
                ...
                editMessage(
                    f"{uname}<i> has sent the following Mega Link</i> - \n<code>{link}</code>\n<i>Engine : </i><b>MegaSDK-REST</b>\n\n<b>User ID</b> : <code>{uid}</code>\n\n",
                    mgs,
                )
                sleep(1)
            else:
                ...
                editMessage(
                    f"{uname}<i> has sent the following Mega Link</i> - \n<code>{link}</code>\n<i>Engine : </i><b>MegaSDK-C++</b>\n\n<b>User ID</b> : <code>{uid}</code>\n\n",
                    mgs,
                )
                sleep(1)
            Thread(
                target=mega_dl, args=(link, f"{DOWNLOAD_DIR}{listener.uid}/", listener)
            ).start()

            if link_type == "folder":
                if isZip:
                    mgs2 = sendMessage(
                        f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴇɢᴀ ғᴏʟᴅᴇʀ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                        bot,
                        update,
                    )
                elif extract:
                    mgs2 = sendMessage(
                        f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴇɢᴀ ғᴏʟᴅᴇʀ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                        bot,
                        update,
                    )
                else:
                    mgs2 = sendMessage(
                        f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴇɢᴀ ғᴏʟᴅᴇʀ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                        bot,
                        update,
                    )
            else:
                if isZip:
                    mgs2 = sendMessage(
                        f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴇɢᴀ ғɪʟᴇ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                        bot,
                        update,
                    )
                elif extract:
                    mgs2 = sendMessage(
                        f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴇɢᴀ ғɪʟᴇ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                        bot,
                        update,
                    )
                else:
                    mgs2 = sendMessage(
                        f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴇɢᴀ ғɪʟᴇ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                        bot,
                        update,
                    )

        # Thread(target=auto_delete_message, args=(bot, update.message, mgs)).start()
        # Thread(target=auto_delete_message, args=(bot, update.message, mgs2)).start()

    elif isQbit and (is_magnet(link) or ospath.exists(link)):
        gmgs = sendMessage("<b>Processing Your Torrent/Magnet...</b>", bot, update)
        sleep(2)
        editMessage(
            f"{uname}<i> has sent the below File/Magnet to QBit Engine</i> :-> \n <code>{link}</code>\n\n<b>User ID</b> : <code>{uid}</code>\n\n",
            gmgs,
        )
        ...
        if reply_to is not None:
            editMessage(
                f"{uname}<i> has sent reply to a TG Media File & sent to QBit Engine</i>\n\n<b>User ID</b> : <code>{uid}</code>\n\n",
                gmgs,
            )
            if isZip:
                gmgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴛᴏʀʀᴇɴᴛ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                gmgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴛᴏʀʀᴇɴᴛ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                gmgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴛᴏʀʀᴇɴᴛ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            ...
            if isZip:
                gmgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴀɢɴᴇᴛ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                gmgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴀɢɴᴇᴛ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                gmgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴀɢɴᴇᴛ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            ...
            if isZip:
                gmgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴛᴏʀʀᴇɴᴛ ғɪʟᴇ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                gmgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴛᴏʀʀᴇɴᴛ ғɪʟᴇ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                gmgs2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴛᴏʀʀᴇɴᴛ ғɪʟᴇ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
        ...
        if len(mesg) > 1:
            ...
            auth = ""
        mssg = sendMessage("<b>Processing Your URL/Torrent/Magnet...</b>", bot, update)
        sleep(2)
        Thread(
        ...
        editMessage(
            f"{uname}<i> has sent the below Link/File/Magnet to Aria Engine</i> :-> \n<code>{link}</code>\n\n<b>User ID</b> : <code>{uid}</code>\n\n",
            mssg,
        )
        if reply_to is not None:
            editMessage(
                f"{uname}<i> has sent reply to a TG Media File & sent to Aria Engine</i>\n\n<b>User ID</b> : <code>{uid}</code>\n\n",
                mssg,
            )
            if isZip:
                mssg2 = sendMessage(
            ...
            or "5:/" in link
            or "6:/" in link
            or "workers.dev" in link
        ):
            if "bots.crunchyflix.xyz" in link or "emily.personalx.in" in link:
                editMessage(
                    f"{uname}<i> has sent our Group`s Index Link & sent to Aria Engine</i>\n\n<b>User ID</b> : <code>{uid}</code>\n\n",
                    mssg,
                )
            if isZip:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ɪɴᴅᴇx ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ɪɴᴅᴇx ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ɪɴᴅᴇx ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            ...
            if isZip:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴜᴘᴛᴏʙᴏx ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴜᴘᴛᴏʙᴏx ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴜᴘᴛᴏʙᴏx ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            ...
            if isZip:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ɢᴏғɪʟᴇ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ɢᴏғɪʟᴇ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ɢᴏғɪʟᴇ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
        elif "mdisk" in link or "mypowerdisk" in link:
            if isZip:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴅɪsᴋ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴅɪsᴋ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴅɪsᴋ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            ...
            if isZip:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴇᴅɪᴀғɪʀᴇ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴇᴅɪᴀғɪʀᴇ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴇᴅɪᴀғɪʀᴇ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
        elif "fichier" in link:
            if isZip:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ғɪᴄʜɪᴇʀ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ғɪᴄʜɪᴇʀ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ғɪᴄʜɪᴇʀ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
        elif "we.tl" in link or "wetransfer.com" in link:
            if isZip:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴡᴇᴛʀᴀsɴғᴇʀ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴡᴇᴛʀᴀsɴғᴇʀ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴡᴇᴛʀᴀsɴғᴇʀ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
        else:
            if isZip:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴅɪʀᴇᴄᴛ-ᴅᴏᴡɴʟᴏᴀᴅ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴀʀᴄʜɪᴠɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            elif extract:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴅɪʀᴇᴄᴛ-ᴅᴏᴡɴʟᴏᴀᴅ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ғᴏʀ ᴇxᴛʀᴀᴄᴛɪɴɢ</b>\n\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
            else:
                mssg2 = sendMessage(
                    f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ᴅɪʀᴇᴄᴛ-ᴅᴏᴡɴʟᴏᴀᴅ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅs</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
                    bot,
                    update,
                )
    ...
    if multi > 1:
        ...
        # Thread(target=auto_delete_message, args=(bot, update.message, mssg)).start()
        # Thread(target=auto_delete_message, args=(bot, update.message, mssg2)).start()
