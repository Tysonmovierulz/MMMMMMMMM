...
def _watch(bot, update, isZip=False, isLeech=False, pswd=None, tag=None, multi=0):
...
def select_format(update, context):
    ...
    if user_id != uid and not CustomFilters._owner_query(user_id):
        ...
        query.answer()
        ytms = sendMessage("<b>Processing Your Link...</b>", bot, query)
        ...
        time.sleep(2)
        editMessage(
            f"<i>Bot has sent the below Link to YT-DLP Engine</i> :-> \n <code>{link}</code>\n\n",
            ytms,
        )
        ydl = YoutubeDLHelper(listener)
        Thread(
        ...
        ytms2 = sendMessage(
            f"<b>ᴛʜᴇ ʀᴇǫᴜᴇsᴛᴇᴅ ʟɪɴᴋ ʜᴀs ʙᴇᴇɴ ᴀᴅᴅᴇᴅ ᴛᴏ ʏᴛ-ᴅʟᴘ ᴇɴɢɪɴᴇ ғᴏʀ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ</b>\n<b>Use /{BotCommands.StatusCommand} ᴛᴏ ᴄʜᴇᴄᴋ ʏᴏᴜʀ ᴘʀᴏɢʀᴇss</b>\n",
            bot,
            query,
        )
        Thread(target=auto_delete_message, args=(bot, query.message, ytms)).start()
        Thread(target=auto_delete_message, args=(bot, query.message, ytms2)).start()
